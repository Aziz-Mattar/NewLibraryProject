...
<div class="container main_and_sec_categories">
    <center>
        <h1 style="line-height:1.3" class="f-s-25">Book Categories</h1>
    </center>
    <div class="m-b-30">
        <div class="form-group search_form_group">
            <form action="{{URL('books_categories')}}" method="get">
    ...
    <div class="the-box">
        <h2><i class="fa fa-book"></i> Book Categories
        </h2>
        <div class="row">
            @foreach($categories as $category)
                <a href="{{URL('books_categories').'/'.$category->name}}">
                    <h3 class="col-lg-3 col-md-4 col-xs-6">{{$category->name}}</h3>
                </a>
            @endforeach
        </div>
    </div>
